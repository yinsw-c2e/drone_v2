...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [localAmapProxy(env), uni()],
    resolve: {
      alias: {
...
function localAmapProxy(env: EnvRecord): PluginOption {
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/__amap/place-text', async (req, res) => {
        const key = amapKey(env);
        if (!key) {
          res.statusCode = 503;
          res.setHeader('content-type', 'application/json; charset=utf-8');
        ...
        const incoming = new URL(req.url ?? '', 'http://localhost');
        const keywords = incoming.searchParams.get('keywords')?.trim();
        if (!keywords) {
          res.statusCode = 400;
          res.setHeader('content-type', 'application/json; charset=utf-8');
          res.end(JSON.stringify({ status: '0', info: 'INVALID_KEYWORDS' }));
          return;
        }
        const upstream = new URL('https://restapi.amap.com/v5/place/text');
        upstream.searchParams.set('key', key);
        upstream.searchParams.set('keywords', keywords);
        upstream.searchParams.set('output', 'json');
        upstream.searchParams.set('city_limit', incoming.searchParams.get('city_limit') || 'false');
        upstream.searchParams.set('page_size', incoming.searchParams.get('page_size') || '8');
        upstream.searchParams.set('page_num', incoming.searchParams.get('page_num') || '1');
        upstream.searchParams.set('show_fields', incoming.searchParams.get('show_fields') || 'indoor,navi');
        try {
          const response = await fetch(upstream);
          const body = await response.text();
          res.statusCode = response.status;
...
function amapKey(env: EnvRecord) {
  return process.env.AMAP_API_KEY
    ...
    || env.VITE_AMAP_API_KEY
    || env.VITE_AMAP_WEB_KEY;
}
